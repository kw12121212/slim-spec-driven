...
它会读取项目上下文和相关 specs，帮助收敛目标与取舍，并自动决定变更名。无需在 proposal 阶段额外确认，就会直接生成与 `/spec-driven-propose` 相同的五个 artifacts；若仍有未决项，会写入 `questions.md`，交给 `/spec-driven-apply` 在实现前作为阻塞项抛出。
...
2. **brainstorm** — 分析模糊想法，收敛范围并自动决定变更名，无需预确认就生成 proposal 产物
...
| 技能 | 功能 |
|------|------|
| `/spec-driven-brainstorm` | 从模糊想法出发，自动收敛范围与变更名，不在 proposal 阶段提问，直接生成完整的五个 proposal artifacts |
...
它会先读取项目上下文和相关 specs，帮助收敛目标、范围与取舍，自动给出
kebab-case 变更名，并直接生成与 `/spec-driven-propose` 相同的五个
proposal artifacts，不在 proposal 阶段额外提问。若仍有歧义，会写入
`questions.md`，由 `/spec-driven-apply` 在实现前结构化阻塞。之后仍可用
`/spec-driven-auto` 执行或用 `/spec-driven-modify` 继续修改。
