...
| AI 读取已有规格 | 明确要求：`brainstorm`、`propose`、`apply` 和 `spec-content` 必须先读 INDEX.md，再读所有相关规格文件，才能生成任何内容 | "搜索已有规格"（模糊） |
...
| 运行时依赖 | 只用 Node.js 标准库，一个约 640 行 TypeScript 文件 | 全局 npm 包（`npm install -g`，需 Node 20.19+） |
